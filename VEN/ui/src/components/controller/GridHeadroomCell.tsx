 ...
 *
 * Clicking a future time on the chart anchors the dashed Import/Export commitment curves at
 * the plan slot it falls in (`GET /flexibility/capacity?start=`, one request per click; the
 * server snaps the time to a slot and names the one it used). Clicking again moves it;
 * clicking at or before now puts the curves back at now. The caption names the start the
 * server actually used, so a tap works the same on touch screens.
 */
export function GridHeadroomCell({
  ...
  const { data: curvesAtStart } = useCapacityCurvesAt(clickedStartMs);
  const anchored = clickedStartMs !== null && curvesAtStart ? curvesAtStart : null;
    ...
    >
      ...
      <Box sx={{ flex: 1, minWidth: CELL_CHART_MIN_WIDTH }}>
        <SiteHeadroomChart
          ...
          onCursorClick={moveCommitmentStart}
        />
        {caption && (
          <Box sx={{ px: 1, pb: 0.5 }}>
