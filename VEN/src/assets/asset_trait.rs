...
pub trait Asset: Send + Sync {
    ...
    /// The power this asset will draw/produce regardless of its commanded
    /// setpoint (e.g. a heater's thermostat forcing emergency heat or a
    /// safety cutoff), or `None` while it follows its setpoint. Controllers
    /// read this (via `AssetSnapshot::forced_power_kw`) instead of
    /// re-deriving the asset's own override rules.
    fn forced_power_kw(&self, _state: &AssetState) -> Option<f64> {
        None
    }
