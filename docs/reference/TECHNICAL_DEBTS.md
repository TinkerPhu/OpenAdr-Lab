...
> **Next ID: R-84.** Use this number for the next new item filed, then increment this
> line to R-85. When resolving and removing an item — even the current highest ID —
...
| R-83 | "Does this window cover t" is written out at many call sites instead of once (found with GB-48, same shape as its interval-timing copies): `AlertWindow`/`SimpleWindow`/`DispatchWindow::is_ended` are three identical impls; `start <= now && now < end` is repeated for tariff lookups (`history_sampler/accumulator.rs`, `monitor.rs`, tick cost), dispatch/alert activity (`sim_tick/dispatch_override.rs`, `arbiter` alert check) and slot overlap `start < slot_end && slot_t < end` in `milp_planner/inputs.rs` (alerts, SIMPLE). Fix: one time-window type (or trait) with `covers(t)` / `overlaps(from, to)` that the three window structs, `TimedInterval`, `CapacitySnapshot` and `TariffSnapshot` share, and a tariff "value at t" lookup beside `tightest_capacity_limit` | `entities/capacity.rs`, `entities/tariff_snapshot.rs`, `controller/event_timing.rs`, `controller/monitor.rs`, `controller/milp_planner/inputs.rs`, `tasks/history_sampler/accumulator.rs`, `tasks/sim_tick/dispatch_override.rs`, `tasks/sim_tick/helpers.rs` | Small | Low | Medium |
