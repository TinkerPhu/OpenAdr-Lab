...
|----|-------------|----------------|--------|------|----------|------|
| R-84 | VTN BFF list routes (`routes/reports.rs`, `events.rs`, `programs.rs`, `vens.rs`) call `VtnClient::get_json` once, so they return at most one openleadr-rs page (50 items) and silently drop the rest; `recorder.rs::fetch_all_pages` already has its own pagination loop for the same lists — a second, divergent copy of "fetch a whole VTN collection". The live Node1 report count is 40; one more report type or reporting event crosses 50. Consolidate into one paginated fetch used by routes and recorder. Found in the fleet-monitor phase 0 analysis (`docs/plans/fleet-monitor/phase-0-foundation.md` F-1). | `VTN/bff/src/routes/*.rs`, `VTN/bff/src/recorder.rs`, `VTN/bff/src/vtn_client.rs` | Small | Low | 🟠 | Medium-High — ends silent truncation in the VTN UI |
| R-85 | Two measurement-report builders with diverging behaviour: the timer path (`reporter.rs::build_measurement_report`, driven by `tasks/sim_tick/publish.rs` from `grid.net_power_w`) and the obligation path (`build_measurement_report_for_obligation`, from `report_intervals.rs::build_net_site_power_ts`). The timer path omits `intervalPeriod`, sends `STORAGE_CHARGE_LEVEL` as a string and `SIMPLE` as a constant 1.0. Consolidate, or delete the timer path once a standing monitoring event exists (fleet-monitor phase 0 F-9 / D-5). | `VEN/src/controller/reporter.rs`, `VEN/src/controller/report_intervals.rs`, `VEN/src/tasks/sim_tick/publish.rs` | Medium | Low | 🟡 | Medium — one net-power derivation for reports and fleet telemetry |
