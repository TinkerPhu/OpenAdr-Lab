...
| R-67 | `react-router`/`react-router-dom` (both `VEN/ui` and `VTN/ui`, pinned `^6.26.0`) has no patched 6.x release for its current advisories — the latest 6.x, `6.30.4`, is itself still in the vulnerable range; only the v7 major line (`7.18.x`) fixes it. A deliberate scope decision made while implementing GB-16 (2026-08-16) — forcing an untested breaking-change major bump into a dependency-hygiene fix would have been worse than leaving it open — tracked here per the R-65↔GB-31 cross-reference pattern; the actionable follow-up itself is backlog item GB-34. | `VEN/ui/package.json`, `VTN/ui/package.json` | Medium (tracked in GB-34) | Low | Low — accepted, documented risk until GB-34 lands |
...
| R-65 | GB-25's `Plan.mip_gap_target` (and its copy on each `plan_history` row) is a proxy only — it persists the solver's *configured* MIP-gap tolerance (`controller::milp_planner::types::MIP_GAP_TARGET`, currently `0.02`), not the *achieved* gap on any given solve. `good_lp`/`highs` expose no achieved-gap query today, so a plan that solved far inside the 2% target looks identical, in this field, to one that just barely made the cutoff. A deliberate scope decision made while implementing GB-25 (see `docs/architecture/VEN_ARCHITECTURE.md` §4.9b), not an oversight — tracked here per that section's cross-reference; the actionable follow-up itself is backlog item GB-31. | `VEN/src/controller/milp_planner/types.rs`, `VEN/src/controller/milp_planner/results.rs`, `VEN/src/entities/plan.rs` | Small-Medium | Low | Low — diagnostic-quality gap, not a functional defect |
