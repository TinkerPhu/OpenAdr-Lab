...
|----|-------------|----------------|--------|------|----------|
| R-19 | `apply_dispatch_override` has the one-tick PV-staleness pattern that `apply_surplus_ev_overlay` was fixed for: it falls back to `AssetSnapshot.power_kw` for uncontrolled assets like PV — last tick's output, not this tick's. The dispatch-override path is **live** (DISPATCH_SETPOINT → `DispatchWindow` → `build_setpoints_and_meta` calls it every tick while a window is active), so a dispatch window active during fast PV ramps steers against stale PV. `SimState::peek_pv_kw` exists and can be threaded through the same way as the overlay fix. | `VEN/src/tasks/sim_tick/helpers.rs` (`apply_dispatch_override`) | Small | Behavioural on an active path | 🟠 |
| R-20 | SITE_RESIDUAL is correctly `grid_kw − Σ modelled_asset_kw`, but the simulator *derives* `grid.net_power_w` as the literal sum of its modelled assets each tick — no independent meter reading exists, so `residual_kw` reads exactly 0 in simulation, always. Makes the WP5.2 exit demo (heuristic MAE < last-known MAE on real fleet history) degenerate for site-residual: both predictors trivially converge to 0. Fix: an unmodelled-load/meter-noise injection in the simulator's grid-meter derivation. | `VEN/src/simulator/mod.rs` (`tick`'s "Derive grid meter" step) | Medium | Low (additive; blocks a meaningful residual-heuristic validation) | 🟠 |
| R-09 | Inject clock into `tasks/planning.rs` instead of calling `Utc::now()` directly: accept a `Fn() -> DateTime<Utc>` in `spawn_planning` so the planning loop is testable without wall-clock coupling. `align_to_step` is already pure. | `VEN/src/tasks/planning.rs` | Small | Low | 🟡 |
| R-10 | Last untyped surface on `VtnPort`: `fetch_reports_raw` returns `Vec<serde_json::Value>` (vtn_port.rs:25) — type or relocate it so no `Value` crosses the port. (`upsert_report` takes typed `OadrReportBody`; `values: Vec<serde_json::Value>` fields stay — OpenADR 3 defines them polymorphic.) | `VEN/src/controller/vtn_port.rs`, `VEN/src/vtn.rs` | Small | Low | 🟡 |
| R-15 | `reportDescriptor.historical` is never parsed, so the VEN cannot distinguish a forecast request from a historical one (the `USAGE_FORECAST` payload itself is served, descriptor-driven). Lands naturally with roadmap WP5.4 (capability forecast + BASELINE). | `VEN/src/controller/openadr_interface.rs`, `VEN/src/controller/reporter.rs` | Small | Low (additive) | 🟡 |
| R-16 | MILP planner samples each slot's tariff at its **start** timestamp only (`interpolate_at(slot_t)`), not the time-weighted mean across the slot — a slot straddling a tariff boundary is priced entirely at the pre-boundary rate. `TimeSeries::time_weighted_mean` (`common/mod.rs`) already exists and fixes this in one call. Same item as `docs/BACKLOG.md` BL-11. | `VEN/src/controller/milp_planner/inputs.rs` | Small | Low | 🟡 |
...
| R-37 | `run_all_tests.sh:24` defaults `DOCKER_HOST=""` (local docker), contradicting the Pi4-only docker rule — `--e2e`/`--resilience` fail on the dev machine unless edited. Default to `Pi4-Server` or read an env override. | `run_all_tests.sh` | Trivial | Low | 🟡 |
...
| ID | Description |
|----|-------------|
| R-40 | File-size near-cap watch (production lines, 2026-07-16): `services/planning.rs` 473/500, `simulator/mod.rs` 470/500, `controller/reporter.rs` 433/500, `milp_planner/results.rs` 415/500, `state/mod.rs` 412/500, `tasks/poll_events.rs` 162/200, `tasks/planning.rs` ~198/200. Split proactively when next touched; `scripts/audit_file_sizes.py` is the authority. |
