...
| R-62 | `entities/site_meter.rs` is entirely dead: all 5 of its types (`DispatchCommand`, `PowerSnapshot`, `SiteMeter`, `DispatchState`, `DeviceSession`) are never constructed anywhere outside their own definitions — confirmed 2026-08-05 by temporarily lifting the file's `#![allow(dead_code)]` and re-running `cargo check --bin ven-app` (full list, not just the 2 originally found while classifying call sites for R-24). `entities/asset.rs::DeviceResponsiveness` (R-65) is the only outside consumer, via this file's own `use` statement — also dead once this file is. Remove the struct(s), or wire them into the actual dispatch-reality tracking they were meant for. | `VEN/src/entities/site_meter.rs` | Trivial | Low | Low — dead-code cleanup, no behavior change |
| R-65 | `entities/asset.rs::DeviceResponsiveness` enum is never constructed or matched anywhere in production or test code — its only reference outside its own definition is a doc comment in `controller/reporter.rs:82` ("mirror of the per-device `DeviceResponsiveness` vocabulary") and the dead `entities/site_meter.rs` (R-62)'s `use` statement. Found 2026-08-05, same audit as R-62/R-63/R-64. Either wire it into `reporter.rs`'s actual per-device reporting (the doc comment implies that was the intent) or delete it. | `VEN/src/entities/asset.rs`, `VEN/src/controller/reporter.rs` | Trivial | Low | Low — dead-code cleanup, no behavior change |
