...
| R-78 | Asset-competence sweep (2026-09-12): `controller/dispatcher.rs::build_setpoints` decides the heater's on/off for a user comfort target itself (`temp_c < target → max_kw`), duplicating `Heater::thermostat_setpoint_kw` (`heater_capabilities.rs`), which only tests call. Needs the target to reach the asset, e.g. the snapshot builder calling `as_thermostat().thermostat_setpoint_kw` when a target is set. | `controller/dispatcher.rs`, `assets/heater_capabilities.rs`, `simulator/snapshot.rs` | Small | Low | 🟡 | Medium — one thermostat authority |
| R-79 | Asset-competence sweep (2026-09-12): `controller/arbiter/arbiter_levers.rs::heater_emergency_lever` reads raw `temp_c`/`temp_min_c`/`temp_safety_max_c` to answer hypothetical-mode questions ("would it be forced on without Curtail?", "could it absorb under Absorb?"); it also ignores the emergency hysteresis the heater itself applies. Needs a heater answer per `HeaterEmergencyMode` (e.g. `forced_power_kw` evaluated under a given mode) so the lever stays available while its own Curtail is active. | `controller/arbiter/arbiter_levers.rs`, `assets/heater.rs` | Medium | Medium (arbiter chatter if done naively) | 🟡 | Medium |
| R-80 | Asset-competence sweep (2026-09-12): MILP test mocks in `controller/milp_planner/tests/mod.rs` hand-roll battery/EV/heater capability from raw SoC/temperature (e.g. `if soc >= 1.0 { 0.0 } else { max_ch }`), so they no longer match the assets (battery 60 s rule, heater emergency). Build them with `services/test_support/asset_snapshots.rs::snapshot_from_asset`. `milp_planner/inputs.rs` also reads the EV's raw `soc` for `soc_ev_init` instead of taking it from the EV's own MILP params. | `controller/milp_planner/tests/mod.rs`, `controller/milp_planner/inputs.rs` | Small | Low | 🟢 | Low-Medium |
