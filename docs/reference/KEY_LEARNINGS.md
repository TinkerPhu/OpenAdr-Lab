...
## A reset script's blast radius should be scoped to what it is resetting (2026-09-19)

`scripts/db_reset.sh` existed to give the VTN a clean schema, and did that by dropping `public`
— correct — and `lab_recorder` alongside it, which is the BFF recorder's history: over a million
`reports_received` rows accumulated across months of fleet runs, regenerated by nothing. It was
reachable from `fleet.sh` and `setup_all.sh --fresh`, whose help text reads "reset the VTN
database" and says nothing about measurement history. Both schemas live in one database, so
"reset the database" quietly meant both, and the two have entirely different recoverability.
The tell was in the script's own comment — `lab_recorder` was described as "re-created on BFF
