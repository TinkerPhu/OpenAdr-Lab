...
| GB-37 | `experiments/run_experiment.py`'s fleet scenarios used to silently run every VEN's EV inert (no active `EvSession` → `EvMilpMode::MustNotRun`), making `energy_business.tariff_response` meaningless. Fixed 2026-08-19/20: (1) `experiments/fleet_ev_roster.json` gives each EV-bearing `ven-1..13` VEN a session spec via `--ev-roster`/`--fleet-map`, bypassing the incompatible `--personas`/`gen_fleet_profiles.py` pipeline; (2) `check_ev_mode_compatible()` rejects an incompatible `--ev-session-mode` for a scenario that declares one (`s9_diurnal.yaml`'s `incompatible_ev_session_modes: [OPPORTUNISTIC, ASAP_FREE]`) before any EV-session HTTP call, closing the "wrong mode silently accepted" gap that a docs-only warning left open; (3) the policy is recorded in `docs/guidelines/FLEET_EXPERIMENT_DESIGN.md` and `s9_diurnal.yaml`'s header. **Still open:** no live S-9 re-run with the new flags has been logged in `docs/history/fleet_run_journal.md` yet to confirm `tariff_response` now produces a sane signal on real data — the fix is implemented and self-check-verified, not yet proven against the live fleet | Low — mechanism is now complete and guarded; what's left is a verification run, not more code | Small — launch S-9 with `--ev-session-mode BY_DEADLINE --ev-roster experiments/fleet_ev_roster.json --fleet-map experiments/fleet_map.json --start-at <next local midnight>`, record the result in `fleet_run_journal.md` |
