...
| [BL-41](#bl-41-device-sessions-api-evheatershiftable-load-direct-control-has-no-ui-surface) | A live, tested backend API for direct EV/heater/shiftable-load control has no UI surface at all — decide keep-and-build or remove | Medium | Unknown until decided | Low — decision-gated, no code risk either way |
...
### BL-41: Device Sessions API (EV/heater/shiftable-load direct control) has no UI surface
**Req:** `VEN/src/routes/hems/ev.rs`, `heater.rs`, `shiftable_loads.rs`, `baseline_override.rs`; `tests/features/ven_device_sessions.feature`; `VEN/ui/src/api/client.ts` (`evSession`/`postEvSession`/`deleteEvSession`, `heaterTarget`/`postHeaterTarget`/`deleteHeaterTarget`, `shiftableLoads`/`postShiftableLoad`/`deleteShiftableLoad`, `baselineOverride`/`postBaselineOverride`/`deleteBaselineOverride`); `VEN/ui/src/api/hooks.ts:486-656` ("Device Session hooks (Phase B)")
**Problem:** Found 2026-08-05 via a `knip` unused-exports sweep of `VEN/ui`: 13 fully-formed React Query hooks (query + mutation, with correct `plan`/self cache invalidation) wrap a live, BDD-tested backend API (`POST/DELETE /ev-session`, `/heater-target`, `/shiftable-loads/{id}`, and the analogous `baseline-override` routes) — but zero page or component in `VEN/ui` imports any of them. Confirmed this isn't dead/superseded code: the backend routes are live and covered by `tests/features/ven_device_sessions.feature` (distinct from the newer unified `user_request` flow, which has its own separate, actively-used `user_request_steps.py` coverage) — so this is a real capability with no way for a user to invoke it via the UI at all, the exact gap the `ui-transparency` rule exists to prevent.
**Fix:** Needs a design decision, not just wiring: does this "Phase B" per-device-type control surface (direct EV session / heater target / shiftable load / baseline override requests) still belong as a first-class UI flow alongside the newer generic user-request mechanism (R-25's `CreateUserRequestBody`), or has the unified flow made it redundant at the *product* level even though the routes remain live and tested? If it's still wanted: add a control surface per device type (likely on each asset's existing detail/control page) that calls these hooks. If superseded: this becomes a removal task instead — delete the unused hooks, and decide separately (with the user, not silently) whether the backend routes/BDD feature should also go.
**Gain:** Medium — either delivers real missing device-control capability, or removes a meaningful chunk (13 hooks + 4 route files + a whole BDD feature) of maintained-but-unreachable surface area, depending on the decision above.
**Complexity:** Unknown until the keep/remove decision is made — Small if removing (delete hooks + update this entry), Medium if building real UI controls for all 4 device types.
**Verify:** If keeping: a UI test asserting each hook is called from some real component, plus a manual E2E check that e.g. creating an EV session via the UI is reflected in the next plan. If removing: confirm `tests/features/ven_device_sessions.feature` is deliberately dropped too (not left orphaned pointing at deleted routes) and `knip` reports zero unused exports in `hooks.ts` afterward.
