...
| GB-32 | `VTN/bff/src/recorder.rs`'s `parse_pt_duration_s` (feeding `report_submission_lag_s`, the SG-3 timeliness `report_lag_s` column archived to `lab_recorder.reports_received`) only recognizes a duration string starting with `"PT"` and silently returns `0` for anything else — but `record_reports` fetches reports via `GET /reports` from the VTN itself, not the VEN's raw POST body, and openleadr-rs's `openleadr_wire::Duration` (backed by the `iso8601_duration` crate) serializes to the fully-qualified `P[n]Y[n]M[n]DT[n]H[n]M[n]S` form (e.g. `"P0Y0M0DT0H5M0S"`), not the VEN's own compact `"PT5M"`. So `parse_pt_duration_s` returns `0` for every real archived report, making `window_end = interval.start` instead of `interval.start + duration` — every `report_lag_s` value ever recorded is off by up to one report interval's length (300–900s in this project's typical configs), silently. All 4 of this function's own unit tests use the compact `"PT15M"`/`"PT5M"` form exclusively, so they pass despite never exercising the real shape of data the function actually receives — a test-fixture gap that masked the bug. Found 2026-08-14/15 reviewing the fleet-run-2 results: `experiments/kpi.py` had the identical bug in its own duration parser (already fixed there, see `docs/history/project_journal.md` and `KEY_LEARNINGS.md`), which prompted checking whether the same pattern existed server-side too — it does | Medium — every SG-3 `report_lag_s` value recorded by this BFF to date is silently wrong by a bounded but non-trivial amount (one report interval), undermining any timeliness analysis or alerting built on this column | Low-Medium — same fix shape as the already-fixed Python side: replace `parse_pt_duration_s` with a full ISO 8601 duration parser (or reuse/extend `openleadr_wire::Duration`'s own parsing if it round-trips), then update the 4 existing unit tests' fixtures to cover the full form (not just add new ones — the existing `"PT..."`-only fixtures should be extended per this project's test-first discipline, not left as the only coverage) |
| GB-33 | `grid_samples.import_limit_kw`/`export_limit_kw` (`VEN/src/history_store/schema.rs` v9) have **never once been non-null**, on any VEN, across the full deployment history checked — confirmed empirically: `SELECT COUNT(*) FROM grid_samples WHERE import_limit_kw IS NOT NULL` returns `0` against ven-1's history.sqlite (47k+ rows spanning the whole deployment, including fleet-run-2's S-3/S-6/S-7 windows, each of which posted a real `IMPORT_CAPACITY_LIMIT`/`RESERVATION` event). The plumbing exists end-to-end — `assets/grid.rs`'s dispatcher tracks `import_limit_kw` (unit-tested), `poll_events/detect.rs` forwards capacity changes into `state::planned_capacity_limits`, and `history_sampler/accumulator.rs:139-140` reads that same state (`capacity_limits: &[CapacitySnapshot]`, matched by `interval_start <= now < interval_end`) to populate the column — but somewhere in that chain the interval never matches at sample time, and the same is true for the export leg. Root cause not yet isolated (deliberately not chased further mid a Python-only KPI-tooling change — this needs a Rust debugging session in its own right). Found 2026-08-15 building `grid_envelope_compliance`/`compliance_latency_s` (the stakeholder KPI reframe's grid-operator-facing block, `experiments/kpi.py`) and regenerating fleet-run-2's `kpis.json` against real data — every VEN's `grid_regulation.import`/`.export` comes back `null` (correctly reported as "nothing to score," per that KPI's own designed behavior for missing data — not a KPI bug), because the underlying capacity-limit column this KPI depends on has silently never worked | Medium-High — this is the data dependency for the single most-requested new KPI in the stakeholder reframe (grid-envelope compliance was called the most pressing question, ahead of energy-business/VEN metrics); until fixed, `grid_regulation` stays empty on every run regardless of how good the KPI code itself is | Medium — needs tracing `CapacitySnapshot`'s `interval_start`/`interval_end` construction (likely in `poll_events/detect.rs` or wherever `changes.capacity_schedule` is built) against the accumulator's `now` comparison to find where the window match fails; add a unit/integration test asserting a real capacity event round-trips into a non-null `grid_samples.import_limit_kw` row, since none of the existing tests (`accumulator.rs`'s own unit tests use synthetic `CapacitySnapshot`s directly, not a real event-to-state pipeline) would have caught this |
