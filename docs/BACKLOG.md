...
| [BL-44](#bl-44-history-tab--split-tariffenvelope-chart--persist-the-capacity-limit-envelope) | History tab shows the same tariff/envelope split as Controller, backed by real persisted capacity-limit data instead of `null` | Medium | M | Low-Medium — genuine schema migration, but mirrors an existing precedent (`pv-curtailment-history`) |
...
### BL-44: History tab — split tariff/envelope chart + persist the capacity-limit envelope
**Req:** `VEN/ui/src/pages/History.tsx`; `VEN/src/routes/hems/history.rs` (`get_history_grid`); `VEN/src/history_store/{schema.rs,mod.rs}` (SQLite-backed archive, distinct from the in-memory per-asset ring buffer in `VEN/src/assets/history.rs`); `VEN/src/tasks/history_sampler/{mod.rs,accumulator.rs}`; `VEN/src/entities/history.rs` (`GridSample` row type)
**Problem:** Explicit follow-up to the Controller-tab tariff/envelope split (2026-08-10, see `docs/history/project_journal.md`). The Controller tab now shows two diagrams — `TariffEnvelopeChart` (direct VTN signals: tariff + capacity-limit envelope) and `GridRatesChart` (derived: cost/CO2 rate) — but the History tab still uses the old combined `TariffChart` with `importLimitKw`/`exportLimitKw` hardcoded to `null` (`History.tsx`'s `tariffPoints` builder). There is genuinely nowhere to source real historical values from: `GET /capacity/schedule` (added alongside the Controller split) only reflects *currently active* events, and `grid_samples` — the SQLite table tariffs are archived into every minute by `HistorySampler` — has no capacity-limit columns (schema is currently at `SCHEMA_VERSION = 8`, no spare columns; confirmed via research 2026-08-10).
**Fix, in dependency order** (persisted data has no consumer until the History tab reads it, so both halves ship together, not separately):
1. **Schema**: add `SCHEMA_V9` in `history_store/schema.rs` — `ALTER TABLE grid_samples ADD COLUMN import_limit_kw REAL; ALTER TABLE grid_samples ADD COLUMN export_limit_kw REAL;` (parallel to the existing `import_tariff_eur_kwh`/`export_tariff_eur_kwh`), bump `SCHEMA_VERSION`, add the `if version < 9` migration branch in `mod.rs::migrate` (mirrors the `pv-curtailment-history` openspec change's SCHEMA_V5 precedent, `openspec/changes/pv-curtailment-history/tasks.md` §5 if still present, else git history around that change).
2. **Accumulation semantics — NOT the tariff mean pattern**: a capacity limit is a ceiling that's usually *absent* (no active event), not a continuous quantity worth averaging over the minute window. Follow the existing PV-curtailment pattern instead (`accumulator.rs`, tightest-value-wins within a priority tier — see `flush_picks_tightest_value_within_same_priority` and neighboring tests) applied to `GridAcc`: track the tightest (lowest) `import_limit_kw`/`export_limit_kw` seen in the window, `None` if no capacity event was ever active during it — never average a limit with an unlimited state.
3. **Wire the sampler**: `HistorySampler::record` needs the current capacity schedule each tick — feed it `state.planned_capacity_limits()` (already populated by `poll_events`, see the Controller-split work) the same way it's already fed `state.planned_tariffs()`.
4. **DB write + query**: extend `GridSample` (`entities/history.rs`) with the two new fields, `append_grid_sample`'s INSERT, and `query_grid`'s SELECT/mapping (`history_store/mod.rs`).
5. **Route**: `GET /history/grid` response gains the two fields automatically once `GridSample` carries them (same struct serializes out).
6. **UI**: swap `History.tsx` to `TariffEnvelopeChart`/`GridRatesChart` (matching Controller), map the new `row.import_limit_kw`/`export_limit_kw` fields into `TariffTimePoint.importLimitKw`/`exportLimitKw` instead of the current hardcoded `null`.
**Gain:** Medium — closes a real gap (envelope data was invisible after-the-fact even though it affects planning decisions in that window) and gives Controller/History visual + data parity.
**Complexity:** Medium — a genuine schema migration + accumulator logic change + UI swap, not a config toggle; sized similarly to the original `pv-curtailment-history` change it mirrors.
**Verify:** accumulator unit tests (tightest-value-wins, None-when-absent, matching the curtailment precedent's test shapes); migration-roundtrip test for `SCHEMA_V9`; `History.test.tsx` updated for the two-chart layout with real (non-null) envelope data from a fixture; manual check that a Controller-tab capacity-limit event, once it rolls into the past, shows up correctly on the History tab.
