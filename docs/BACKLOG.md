...
| GB-18 | VTN recorder's `report_lag_s`/`reports_received` data appears empty for the exact wall-clock window of any `experiments/run_experiment.py` scenario run — confirmed independently across two full S-1..S-6 runs (2026-08-09/10, see `docs/history/project_journal.md`) by grepping `recorder-reports_received.csv` directly for each run's `[started_at, started_at+duration)` window: zero matching rows both times, despite the CSV holding ~120k historical rows. Blocks WP5.4/SG-3 report-usefulness evaluation from ever seeing live experiment data. Needs investigation: is the recorder's `received_at` timestamp using a different clock/timezone than the experiment's `started_at`, or is it genuinely not being invoked during these windows? | Medium — blocks SG-3 rigor (WP5.4) | Medium — needs recorder-side investigation |
| GB-19 | `ven-3` (and its persona-fleet counterpart role `fleet-ven-002`) shows a reproducibly larger response than `ven-2` to the same S-5 `DISPATCH_SETPOINT` event, across two independent live S-1..S-6 runs (2026-08-09/10): ~6.6 kW peak import / ~0.89 kWh vs. `ven-2`'s steady ~0.5 kW / ~0.24 kWh, both times. Reproducing across two independent runs rules out one-off noise. Not yet root-caused — check whether it's a genuine per-VEN planner response difference (different asset mix/profile) or a scenario/harness artifact specific to `ven-3`'s profile | Low-Medium — could indicate a real planner behavior difference worth understanding, or a harness bug | Low — investigation only, no fix scoped yet |
