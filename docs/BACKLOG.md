...
### BL-44: History tab — split tariff/envelope chart, historical capacity-limit schedule
**Req:** `VEN/ui/src/pages/History.tsx`; `VEN/src/routes/hems/misc.rs` (`get_history_grid`); `VEN/src/entities/history.rs` (grid ring buffer)
**Problem:** Explicit follow-up to the Controller-tab tariff/envelope split (2026-08-10, see `docs/history/project_journal.md`). The Controller tab now shows two diagrams — `TariffEnvelopeChart` (direct VTN signals: tariff + capacity-limit envelope) and `GridRatesChart` (derived: cost/CO2 rate) — but the History tab still uses the old combined `TariffChart` with `importLimitKw`/`exportLimitKw` hardcoded to `null` (`History.tsx`'s `tariffPoints` builder), since the grid history ring buffer that feeds it has no capacity-limit columns and there's no historical equivalent of `GET /capacity/schedule` (which only reflects currently-active events, not what limits applied to past intervals).
**Fix:** (1) Swap `History.tsx` to `TariffEnvelopeChart`/`GridRatesChart` the same way Controller did. (2) Decide + implement how past capacity limits are sourced: either persist `CapacitySnapshot` rows into the grid history ring buffer alongside tariff (mirroring how tariff history already works), or accept that History only ever shows `null` envelope data before the feature existed and starts populating going forward.
**Gain:** Low-Medium — visual consistency between Controller and History tabs; real historical envelope data is a nice-to-have, not blocking.
**Complexity:** Small (chart swap) + Medium (historical persistence, if done) — can ship the swap alone first.
**Verify:** `History.test.tsx` updated for the two-chart layout; if historical persistence is added, a fixture asserting a past capacity-limit change shows up in the History chart.
