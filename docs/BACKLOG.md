...
| GB-37 | `experiments/run_experiment.py`'s fleet scenarios silently run every VEN's EV inert: without an active `EvSession`, `VEN/src/assets/ev_milp.rs::EvMilpContext::from_state` returns `EvMilpMode::MustNotRun` (zero charging power, every slot), so the EV just sits at `initial_soc` for the whole run. The S-9 24h diurnal re-run (2026-08-17/19, `docs/history/fleet_run_journal.md`) hit exactly this and its `energy_business.tariff_response` correlation looked scattered/random fleet-wide because every EV-touching VEN's number was measuring an inert EV, not price response. `--personas` looked like the fix but **is not currently usable against this fleet at all**: it requires `VEN/fleet/manifest.json` (default `--fleet-manifest` path), which does not exist in this repo — that manifest/persona pipeline belongs to `scripts/gen_fleet_profiles.py`'s independently-*generated* fleet (writes its own `VEN/fleet/manifest.json` + `VEN/docker-compose.fleet.yml`), never wired to the hand-authored `VEN/profiles/ven-1..13.yaml` fleet that `experiments/fleet_map.json` and every S-1..S-10 scenario actually target. Even where `--personas` *is* usable (the generated fleet), it wouldn't fully fix a tariff-response test either: `scripts/personas.py`'s `"eco"` preset uses `ev_mode: "OPPORTUNISTIC"`, whose `from_state` arm sets `free_only: true` — `inject_grid_slots` gates charging on PV-surplus-or-nonpositive-price slots only, rewarded via `v_ev_free_charge_eur_kwh`, never the real grid tariff. Only `BY_DEADLINE`/`ASAP` sessions route the actual per-slot tariff into the EV's MILP reward | Medium — a scenario intended to test realistic price-response has no working path to a responsive EV today, on either fleet, and produced a misleading KPI on first use, not a crash or error | Medium — (1) hand-author a small `VEN/fleet/manifest.json`-equivalent for `ven-1..13` giving each EV-bearing VEN a session spec, since they're not `gen_fleet_profiles.py` output and can't just reuse its manifest format as-is without a compatibility check; (2) change the session-setup path so a tariff-response-focused scenario forces every EV-bearing VEN onto a single tariff-sensitive mode (`BY_DEADLINE` with a realistic deadline, or `ASAP`) — never `OPPORTUNISTIC`/`ASAP_FREE`, and never a mixed-mode persona spread, so `tariff_response` measures one controlled behavior, not a blend of solar-seeking and price-seeking assets; (3) record the resulting policy in `docs/guidelines/FLEET_EXPERIMENT_DESIGN.md` and reference it from `s9_diurnal.yaml`'s own header comment, the same convention already used there for timing guidance |
