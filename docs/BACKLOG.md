...
| GB-30 | No automated code-coverage tooling for this project's own code (VEN/VTN). `.claude/CLAUDE.md` states "No enforced coverage floor — keep domain and application layer tests meaningful", so coverage is judged qualitatively, not measured; the only `codecov.yml`/coverage config in the repo tree belongs to the `openleadr-rs` submodule's own upstream CI (`openleadr-rs/.github/codecov.yml`), unrelated to and not consumed by this project. Confirmed 2026-08-13 while answering a coverage question — no coverage report or percentage exists anywhere to cite. **2026-08-13: opt-in tooling added** — `run_all_tests.sh --coverage` runs a new `ven-coverage` docker service (`tests/docker-compose.ven-unit-test.yml`, `tests/Dockerfile.ven-coverage`, own cache volumes, HTML+JSON output to `coverage/ven/` on the docker host) — never part of the bare/no-flag "everything" run, per the recompile-cost/RAM reasoning below. **Not yet run** — first invocation on Node1/Node2 (needed to verify the image builds and tarpaulin actually completes) and the resulting-report analysis are still outstanding | Low — visibility/process improvement, not a known defect; existing gaps are already tracked individually where found (e.g. R-33, GB-12) | Low — remaining work is verification only: run `run_all_tests.sh --coverage` once on Node1/Node2 under the usual lock, confirm the HTML/JSON report is produced and the % looks sane, then use the JSON output for the planned results analysis. Reasoning for staying opt-in: (1) `cargo tarpaulin`'s instrumented build/run is itself 2–4× slower than plain `cargo test`; (2) more importantly, tarpaulin's instrumented build uses different `RUSTFLAGS`/compile settings than a normal `cargo build`/`test`, so it does not share cargo's incremental-compilation cache — every time the flag is toggled on or off, the *next* build is a full from-scratch recompile of the whole dependency graph (not incremental), which is expensive on this project's already cmake/HiGHS-heavy VEN Rust build. Also mind the `memory-budget`/`wsl-lock` constraints (8 GB laptop, WSL builds already crash on pagefile exhaustion) if ever run locally rather than on Node1/Node2 |
