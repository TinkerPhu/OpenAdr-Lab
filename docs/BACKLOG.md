...
| GB-25 | Plan-quality history isn't persisted anywhere: `solver_ms` exists only on the transient `plan_ready` SSE event, never on the persisted `Plan`/`GET /plan`; `PlanWarning` is free-text (`message: String`), no typed `kind` for deadline-miss/comfort-band-exit/unmet-SoC/etc.; no MILP optimality-gap field exists at all; and `plan_snapshots` (the one table that would have stored plan history over time) was dropped as dead code (R-63) with no replacement. Found 2026-08-12 while designing the fleet-experiment diagnostics recording (`docs/history/project_journal.md`'s "Full 13-VEN fleet deploy + S-1..S-6" entry) — worked around there via an external `GET /plan` poller rather than building this, per an explicit user scope choice to reuse what exists this pass | Medium — blocks any future post-hoc "why did this VEN's plan look like that at 14:32 last Tuesday" investigation; today only live-polling can see plan quality at all | Medium — touches `entities/plan.rs`, `services/planning.rs`, and a `history_store` schema migration; real feature work, not a one-line fix |
| GB-26 | `experiments/run_experiment.py`'s `snapshot()` dumps the *entire* `lab_recorder.reports_received`/`events_published`/`ven_snapshots` tables via `COPY ... TO STDOUT`, unfiltered by the run's time window. On this deployment's accumulated history that's already ~122 MB for `reports_received` alone (hit in a 3-minute smoke-test dry run 2026-08-12, and required a `kpi.py` `csv.field_size_limit` fix — see journal entry above — just to read it back). Grows unboundedly with deployment age; every scenario run pays the full dump cost regardless of how short the run is | Low-Medium — no functional bug (kpi.py's window filtering still produces correct KPIs), but disk/time cost of every experiment run scales with total deployment history, not run length | Low — filter the `COPY` query by `received_at`/timestamp using the run's `t_from`/`t_to` (already computed) instead of dumping the whole table |
| GB-27 | Scenario runs via `experiments/run_experiment.py` never come back with `report_timeliness`/`event_impact_kwh` populated (`kpi.py` prints `null`) because the scenario YAMLs (`experiments/scenarios/s1..s6`) never set up a `reportDescriptor` requesting `BASELINE`/`USAGE` reports on the program/event they create — confirmed 2026-08-12 across all of S-1..S-6. WP5.4's BASELINE-report M&V machinery (shipped 2026-08-11) is real and works (verified live against `ven-1`), it's just never exercised by the scripted experiment scenarios themselves | Medium — SG-3's "report usefulness" KPI (`docs/plans/strategic_roadmap.md` §4) is unverified by the scripted scenario matrix, only by a one-off manual demo | Low-Medium — either extend the scenario YAML schema with an optional `report_descriptor` action, or add a `setup_persona_sessions`-style report-subscription step to `run_experiment.py` |
| GB-28 | `energy_shifted_kwh` (`kpi.py`, vs. an `--baseline` run) is confounded by time-of-day when the baseline and the scenario run don't happen at the same wall-clock time — the sim clock has no injectable-time acceleration for this harness, so scenarios run in real time and back-to-back runs land hours apart. Concretely observed 2026-08-12: `ven-1` (PV-equipped) showed `energy_shifted_kwh = -0.36 kWh` (i.e. "shifted worse") during an S-4 grid-emergency run vs. the S-1 baseline, despite `solve_status: OPTIMAL` and zero warnings/violations throughout — very likely explained by S-1 running at 21:57 UTC and S-4 at 06:21 UTC the next day (see journal entry), not an actual emergency-response failure | Medium — makes `energy_shifted_kwh` unreliable evidence for any single VEN across non-simultaneous runs; fleet-aggregate KPIs and same-run comparisons (e.g. warnings/violations) are unaffected | Medium — needs either a paired/simultaneous control VEN excluded from the event, or same-time-of-day scheduling across baseline/scenario runs, to remove the confound |
| GB-29 | `experiments/run_experiment.py`'s docstring says it "Runs ON the docker host (Node1), same convention as `fleet.sh`" — no longer true for a multi-host (Node1+Node2) run: Node1 has no configured ssh trust to Node2 (`ssh Node1 "ssh Node2 ..."` fails to resolve the alias; agent-forwarding also failed with `Permission denied (publickey)`), so the 2026-08-12 full-fleet run was orchestrated from an off-host workstation instead (`--fleet-map`/`--pg-host` routing every VEN, including Node1's own, through its ssh alias). Works, but the docstring/design intent is now stale | Low — cosmetic/documentation drift, the workaround is fully functional | Low if just updating the docstring to document the off-host pattern; Medium if the intent is instead to restore Node1-to-Node2 ssh trust and keep on-Node1 orchestration as the primary mode (a deliberate choice, not a default to make unprompted) |
