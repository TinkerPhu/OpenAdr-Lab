...
| GB-30 | No automated code-coverage tooling for this project's own code (VEN/VTN). `.claude/CLAUDE.md` states "No enforced coverage floor — keep domain and application layer tests meaningful", so coverage is judged qualitatively, not measured; the only `codecov.yml`/coverage config in the repo tree belongs to the `openleadr-rs` submodule's own upstream CI (`openleadr-rs/.github/codecov.yml`), unrelated to and not consumed by this project. Confirmed 2026-08-13 while answering a coverage question — no coverage report or percentage exists anywhere to cite | Low — visibility/process improvement, not a known defect; existing gaps are already tracked individually where found (e.g. R-33, GB-12) | Low — if added, make it an opt-in step (e.g. a `run_all_tests.sh --coverage` flag or a periodic/CI-only job), not part of the default suite every commit runs, for two reasons: (1) `cargo tarpaulin`'s instrumented build/run is itself 2–4× slower than plain `cargo test`; (2) more importantly, tarpaulin's instrumented build uses different `RUSTFLAGS`/compile settings than a normal `cargo build`/`test`, so it does not share cargo's incremental-compilation cache — every time the opt-in flag is toggled on or off, the *next* build is a full from-scratch recompile of the whole dependency graph (not incremental), which is expensive on this project's already cmake/HiGHS-heavy VEN Rust build. Also mind the `memory-budget`/`wsl-lock` constraints (8 GB laptop, WSL builds already crash on pagefile exhaustion) if run locally rather than on Node1/Node2 |
