...
| Report payload unit semantics (`USAGE` vs `DEMAND`) | — | Partial | `USAGE` carries mean site power in W, but the spec defines `USAGE` as energy over the interval and `DEMAND` as real power. The fleet monitor adds a spec-correct signed `DEMAND` series in kW with `payloadDescriptors` (`docs/plans/fleet-monitor/phase-0-foundation.md` §5); `USAGE`-as-power stays for `exp-*` events until `experiments/kpi.py` and the reporter BDD steps move to energy, which is its own change |
| `reportDescriptor` fields honoured | — | Partial | `payloadType`, `readingType`, `historical` and `frequency` are read; `frequency` is interpreted as seconds rather than "number of intervals between reports", and `startInterval`, `numIntervals`, `repeat`, `aggregate` are ignored (`openadr_interface.rs::extract_report_obligations`) |
