...
| **Device Session** | — | A schedulable unit of energy delivery for a specific asset: a fixed amount of energy (kWh, or an equivalent target such as SoC or temperature) and a deadline. Represented per asset type — `EvSession` (target SoC + departure time), `HeaterTarget` (target temperature + ready-by time), `ShiftableLoad` (fixed power × duration within an earliest/latest window) — rather than one generic type. Enters the MILP as a deadline constraint (`MilpLoadMode::MustRun`/`MayRun`), not as an iterated lifecycle object; there is no shared status field across asset types. See `entities/device_session.rs`. |
