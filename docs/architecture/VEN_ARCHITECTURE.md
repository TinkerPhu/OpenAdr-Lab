...
- **Capacity Forecast from a future start** (Controller → Site Headroom, click a
  future time — one request per click, `GET /flexibility/capacity?start=`) —
...
| B — frozen + EMA return | `pv_irradiance` (+`pv_tau_s`), `base_load_kw` (+`base_load_alpha`) | Held while active; EMA-blended back to the natural model on release |
