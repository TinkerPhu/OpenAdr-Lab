...
since fixed, see §7 scenario B), and the replan trigger had to switch from raw deviation to
...
- **No PV forecast in the plan itself, at the time.** `deviation_kw` was structurally dominated by
  ...
  PV through the weather forecast (§7 scenario B) — but a new correction layer still needs the
...
plan itself didn't yet model. The PV gap is closed now (§7 scenario B); the arbitration gap is not.
§6 proposes a concrete design for the arbitration problem: a single arbiter that subsumes the
...
## 3. Asset physical limits vs. current simulation/control limits
...
"lever" in §5's table below and in §6's marginal-cost design. This matters directly for deviation
...
| **Heater / boiler** | Slow, thermal-inertia bound; two-tier bounds (see §3) | Both `temp_min_c`/`temp_max_c` are comfort-level, not physical; the true safety envelope (ambient floor, real ceiling above `temp_max_c`) is wider and currently unmodelled | `temp_max_c`/`temp_min_c` (comfort band, today's only bounds), no field yet for the true safety ceiling; relay-wear concerns (`min_state_linger_s`, proven useful in feature 017, not currently implemented) |
...
## 6. Marginal-cost single-arbiter design (proposed) — replaces the per-objective priority table
...
### 6.1 The signal is per-slot marginal cost, not "which slots are minimizing vs. maximizing import"
...
### 6.2 Getting the number out of the solver
...
maximization, or autarky if ever added as a soft objective — §6 stays valid there, see the
...
### 6.3 One arbiter, one owner of every actuator write per tick
...
### 6.4 Worked examples across the deviation catalog (§4)
...
zero-cost lever (§3: no physical floor) and gets used opportunistically — not because a static rule
...
heater below today's `temp_min_c` (§3) toward ambient — not because a special "DR event" row
...
### 6.5 Where pure greedy breaks — the SoC-coupling trap
...
### 6.6 Preconditions before this can be built

- Requires the `SolverPort` output extension in §6.2 (one extra cheap LP solve per cycle).
- Requires collapsing the opportunistic EV-surplus overlay and any real-time corrector into the one
  arbiter described in §6.3 — building this as a second independent loop reproduces 017's failure
  regardless of how the priority number is computed.
- The plan-must-forecast-PV precondition once listed here is already satisfied (§7 scenario B) — but
  §7 B's own open point still applies: no experiment has confirmed how much that forecast actually
  reduces PV-driven deviation, which is worth checking before relying on the resulting shadow prices.

## 7. Ideal vs. realistic vs. "where it fits in the VEN"

| Scenario | Ideal response | Realistic response given current architecture | Where it fits |
|---|---|---|---|
| A. PV cloud transient | Fast lever absorption per active objective, potentially including PV curtailment once the planner can choose it (§3/§8 task 2) | Only battery/EV can react today; the historically-tried absorber for this was removed for oscillating with the opportunistic EV overlay (§1) — a rebuild must fix that arbitration gap first, not just re-tune dead-bands | The §6 marginal-cost arbiter, once (a) it subsumes the opportunistic overlay and reads current-tick PV, and (b) the `SolverPort` duals (§6.2) exist |
...
| C. Inverter clipping | Planned inside the export limit; if not, hard ceiling, no lever needed (today) — an active lever once the planner can choose `export_limit_kw` itself (§8 task 2) | Export limit enforced as a clamp in `dispatcher.rs`; the clamp mechanism already works (§3), it's just never planner-driven | MILP input validation today; active curtailment lever once the planner gains that decision variable |
| D. Base load step (washing machine) | Absorbed by whichever lever the objective prefers; the load itself never participates | Same arbitration-gap caveat as scenario A | The §6 marginal-cost arbiter, same prerequisites as scenario A |
...
| H. VTN event boundary mismatch | Fast, obligation-aware absorption allowed to override normal priority/comfort bounds (including heater's comfort floor, §3/§6.4) for the event's duration | No obligation-aware override path exists | New: the §6 marginal-cost arbiter, with the obligation's breach penalty baked into that window's shadow price (§6.4) — not a separate mode, tied to WP-T5 report-submission-status tracking |
...
| J. Heater comfort-floor recovery | Should be a policy choice bounded by the comfort band, not an unconditional immediate override into the (currently unmodelled) safety envelope (§3) | Currently unconditional (`emergency_active` bypasses everything) | Add the true safety floor/ceiling as a second, wider bound pair — reachable only under an active VTN emergency directive — plus the corresponding profile fields (§3) |
...
- Arbitration: a single arbiter owns every actuator write per tick, subsuming the opportunistic
  overlay (§6.3) — folded into task 4 below, not a standalone design question.
- Objective-conditional priority/override rules: fall out of the marginal-cost/shadow-price signal
  (§6) rather than separate per-objective priority tables — folded into tasks 3–4 below.
...
1. **Add the heater's true safety envelope** (§3) — `temp_min_c`/`temp_max_c` stay as the comfort
   band; a new safety floor/ceiling (ambient / a real hard ceiling above `temp_max_c`) is added,
   reachable only under an active VTN emergency directive, plus the corresponding new per-profile
   fields (`VEN/profiles/*.yaml`). Trivial effort, no dependencies.
2. **Give the planner a PV-export decision variable** (§3) — the simulator already curtails
...
3. **`SolverPort` marginal-cost/shadow-price extension** (§6.2) — one extra cheap LP solve per
   planning cycle, exposing directional duals per slot. Only useful once building the arbiter, so it
   comes right before it.
4. **Build the single arbiter** (§6.3) — the highest-risk piece, the one that's failed twice
   already (§1). Do it last, so it inherits the richer lever set from tasks 1–2 and whatever's
   learned building task 3; give it its own focused design pass rather than rushing it alongside
   everything else.
5. **Clean up the three confirmed leftover references** (§2) — trivial, do anytime, lowest priority.
