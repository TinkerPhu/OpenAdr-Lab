...
8. From a shell (any machine that reaches Node1):

   ```bash
   # Create an EV session with an explicit mode
   curl -s -X POST http://Node1:8211/ev-session \
...
9. `curl -s http://Node1:8211/ev-session` — **Expected:** `"mode":"ASAP"` echoed.
10. Delete it: `curl -s -X DELETE http://Node1:8211/ev-session`.
...
12. `curl -s http://Node1:8211/user-requests` after creating a request from the
...
10. `curl -s http://Node1:8211/plan | jq '[.slots[] | {start, ev: (.allocations[]? | select(.asset_id=="ev") | .power_kw)}]'`
...
5. Stop the VTN container: `ssh Node1 "docker stop vtn-vtn-1"` (test stack
...
8. `curl -s http://Node1:8211/notifications | jq length` — note the count.
9. Restart VEN1: `ssh Node1 "cd /srv/docker/openadr_lab/VEN && docker compose restart ven-1"`.
...
11. `curl -s "http://Node1:8211/notifications?since=2026-07-12T00:00:00Z"`
    returns only entries newer than the timestamp.
12. `curl -N http://Node1:8211/notifications/events` holds an SSE stream
...
1. On the **VTN UI**, publish a short price event: three intervals over the
   ...
   coverage.
2. After the next replan: `curl -s http://Node1:8211/plan | jq '[.slots[] | {start, rate: .import_tariff_eur_kwh, est: .rate_estimated}] | .[0:8]'`
...
3. `curl -s http://Node1:8211/plan | jq .warnings`
...
4. Install an override:
   ...
   ```bash
   curl -s -X POST http://Node1:8211/assets/ev/comfort_curve \
     -H 'Content-Type: application/json' \
...
5. `curl -s http://Node1:8211/assets/ev/comfort_curve` echoes the
   override.
...
8. `curl -s -X DELETE http://Node1:8211/assets/ev/comfort_curve` →
...
4. Pin the PV forecast to zero so no energy is free:
   `curl -s -X POST http://Node1:8211/sim/inject -H 'Content-Type: application/json' -d '{"pv_plan_kw": 0.0}'`
...
6. Restore PV (`curl -s -X POST http://Node1:8211/sim/inject/reset`) at
...
> All commands run on the Node1 docker host (`/srv/docker/openadr_lab`), in a
...
6. API cross-check: `curl -s http://Node1:8211/signals | jq keys` →
