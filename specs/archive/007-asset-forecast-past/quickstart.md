...
| File | Change |
|---|---|
| `VEN/src/common/mod.rs` | **NEW FILE**: `Interpolation`, `Quantity`, `Unit`, `TimeSeries` — add `mod common;` to `main.rs` |
...
return TimeSeries { samples, interpolation: Linear }
```

### 3. Remove `pv_forecast()` from planner.rs

Add `asset_forecasts: &HashMap<String, TimeSeries>` to `run_planner()` and `build_grid()`. In `build_grid()`, replace:
```rust
...
```rust
let asset_forecasts: HashMap<String, TimeSeries> = sim_state.assets
    .iter()
