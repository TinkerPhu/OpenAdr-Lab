...
- [ ] T001b [P] Audit BDD coverage for acceptance scenarios — open each of `tests/features/` feature files and confirm at least one scenario exercises the 6 named functions (`build_setpoints`, `apply_surplus_ev_overlay`, `apply_battery_correction_overlay`, `apply_deviation_absorption`, `record_tick`, `compute_envelope`) end-to-end via the HTTP/tick path. If no scenario covers a function, add a minimal scenario to an appropriate `.feature` file (e.g., `tests/features/controller.feature`). See CX-002 in plan.md for rationale. *(Constitution Principle II compliance task.)*
...
- [ ] T005 Edit `VEN/src/simulator/mod.rs` — **first** run `grep -n "pub struct AssetHistoryBuffer" VEN/src/simulator/mod.rs` to confirm it returns no matches (pre-condition: FR-004 structurally satisfied). Then remove the struct definitions for `SimSnapshot` (line ~400), `AssetSnapshot` (line ~381), and `GridSnapshot` (they now live in `controller/simulator_port.rs`). Add re-export aliases: `pub use crate::controller::simulator_port::{SimSnapshot, AssetSnapshot, GridSnapshot};` so existing `use crate::simulator::SimSnapshot` imports keep compiling without change. Verify `cargo build` succeeds.
