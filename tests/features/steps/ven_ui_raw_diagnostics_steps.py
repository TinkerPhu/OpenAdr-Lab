...
def step_see_cell(context, title):
    el = context.browser_page.wait_for_selector(
        tid(f"diagnostic-cell-{_slug(title)}"), timeout=20000
    )
...
def step_click_refresh(context, title):
    btn = context.browser_page.wait_for_selector(
        tid(f"refresh-btn-{_slug(title)}"), timeout=20000
    )
    btn.click()
...
def step_sim_cell_has_content(context):
    ...
    sim_cell = page.wait_for_selector(tid("diagnostic-cell-simulator-state"), timeout=20000)
...
def step_select_timeline_series(context, series):
    page = context.browser_page
    select = page.wait_for_selector(tid("timeline-series-select"), timeout=20000)
    select.click()
...
def step_series_dropdown_visible(context):
    el = context.browser_page.wait_for_selector(
        tid("timeline-series-select"), timeout=20000
    )
