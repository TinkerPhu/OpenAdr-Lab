...
def _expand_ev_right_section(page):
    ...
    # Wait for asset cells to appear first (React Query may still be fetching)
    page.wait_for_selector("[data-testid^='asset-cell-']", timeout=25000)
    btn = page.wait_for_selector(
        tid("asset-cell-ev-collapse-right"), timeout=25000
    )
...
def _expand_ev_status_accordion(page):
    ...
    page.wait_for_selector(tid("right-section-ev"), state="visible", timeout=15000)
