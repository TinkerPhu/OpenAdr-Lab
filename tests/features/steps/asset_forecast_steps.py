...
def step_get_history(context, asset_id, timespan_s):
    ...
    r = ven_get(f"/history/{asset_id}", params={"timespan_s": timespan_s})
    context.last_response = r
    if r.status_code == 200:
        context.history_json = r.json()
    else:
        context.history_json = None
...
# ── Then: history (GET /history/:asset_id) ────────────────────────────────────

@then("the history response has a non-empty samples list")
def step_history_non_empty(context):
...
@then("the history response is a valid history response")
def step_history_valid(context):
    data = context.history_json
    assert data is not None, "No history JSON in context"
...
def step_history_ascending(context):
    samples = context.history_json.get("samples", [])
    timestamps = [s["ts"] for s in samples]
