# notification-history-viewer — delta spec
...
UI consumers of the SSE stream SHALL treat an incoming notification whose `id`
already exists in the displayed list as an update (replacing count and
last_seen_at) rather than appending a duplicate entry.

#### Scenario: SSE re-emission updates the existing row
- **GIVEN** the bell feed shows a notification with `count` 3
- **WHEN** an SSE message arrives with the same `id` and `count` 4
