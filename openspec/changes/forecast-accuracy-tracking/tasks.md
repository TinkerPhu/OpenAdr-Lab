...
- [ ] 1.1 Add `ForecastLeadKind` enum (`Near | Far`) to `VEN/src/entities/history.rs`, with
      `Display`/`FromStr` (or equivalent) for the `"near"`/`"far"` TEXT mapping, mirroring
      `curtailment_source`'s string convention
- [ ] 1.2 Add `ForecastAccuracySample` struct: `asset_id: String`, `lead_kind: ForecastLeadKind`,
      `target_ts: DateTime<Utc>`, `predicted_kw: f64`, `predicted_at: DateTime<Utc>`,
      `actual_kw: Option<f64>`, `actual_at: Option<DateTime<Utc>>`
- [ ] 1.3 Unit tests: `ForecastLeadKind` round-trips through its string form; both valid values
      parse, an invalid string is rejected

## 2. History store: schema + port

- [ ] 2.1 Add schema v8 (`VEN/src/history_store/schema.rs`): `CREATE TABLE
...
- [ ] 2.2 Add to `HistoryPort` trait (`VEN/src/controller/history_port.rs`):
...
- [ ] 2.3 Implement all three on `SqliteHistoryStore` (`VEN/src/history_store/mod.rs`)
- [ ] 2.4 Extend `prune_before` to also delete `forecast_accuracy_samples` rows with `target_ts <
      cutoff`
- [ ] 2.5 Implement on `MockHistoryPort` (`VEN/src/services/test_support/mock_history_port.rs`)
- [ ] 2.6 Unit tests: append then query round-trips predicted-only rows; reconciliation fills
...
- [ ] 3.1 In `VEN/src/services/forecast.rs`, add `record_forecast_accuracy_samples(plan: &Plan,
...
- [ ] 3.2 Wire into `finish_plan_cycle`/`publish_post_cycle_state`: call the builder, then
...
- [ ] 3.3 Unit tests: a 2+ slot plan yields exactly 6 samples (3 assets × 2 points); a
...
- [ ] 4.1 In `tasks/history_sampler/mod.rs::write_window`, after `history.append_tick_samples`,
...
- [ ] 4.2 Unit test: a flushed window containing an asset with an open forecast sample whose
      `target_ts` falls inside it results in that sample being reconciled after the flush

## 5. Query route

- [ ] 5.1 Add `GET /history/forecast-accuracy?from=&to=&asset_id=&lead_kind=` to
...
- [ ] 5.2 Wire the route in `routes/mod.rs`
- [ ] 5.3 Route tests: valid range returns rows; invalid `lead_kind` value returns 400; disabled
      history store returns 503, matching the existing routes' behavior

## 6. VEN UI

- [ ] 6.1 Add an API client method + hook for `GET /history/forecast-accuracy` (VEN UI's existing
      history-fetching pattern)
- [ ] 6.2 In `AssetTimelineChart.tsx`, overlay the near and far series (two additional lines,
      visually distinct from the actual line and from each other) for the PV, base_load, and
      site-residual cells only
- [ ] 6.3 UI unit tests: overlay renders both series when data is present; renders cleanly with no
      overlay when the query returns no rows (asset not yet reconciled, or endpoint unavailable)

## 7. Documentation & cleanup

- [ ] 7.1 Update `DOCUMENTATION.md` (new history table, new route, schema version)
- [ ] 7.2 Append `docs/history/project_journal.md` entry
...
- [ ] 8.1 `wsl cargo check` / `wsl cargo test -p ven-app` locally (wsl_lock)
- [ ] 8.2 `cargo fmt --check` and `cargo clippy --all-targets --all-features -- -D warnings`
- [ ] 8.3 `scripts/audit_file_sizes.py`
- [ ] 8.4 VEN UI unit tests + `npm run build`
