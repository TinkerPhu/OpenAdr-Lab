...
(045) literally the same `StackedAreaChart`/`StackedTimeSeriesChart` component — draws the
grid line correctly for the same plan.
...
- No change to `StackedAreaChart`/`StackedTimeSeriesChart` rendering itself, to the
  `/timeline/all` backend endpoint, or to the MILP planner's `net_import_kw`/`net_export_kw`
  semantics — this is purely a frontend data-sourcing fix on the Planner side.
...
- No visual change to the Controller tab; the Planner tab's grid line changes from
  (incorrectly) near-zero to correctly negative during export-heavy autarky slots.
