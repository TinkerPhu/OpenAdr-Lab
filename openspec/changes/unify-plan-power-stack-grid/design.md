...
render `StackedAreaChart`/`StackedTimeSeriesChart` — the same component since
`unified-chart-primitives` (045) — but feed it from two independently-written data
builders:
...
**Non-Goals**
- Do not touch `StackedAreaChart`/`StackedTimeSeriesChart` rendering, `axisDomain.ts`, or
  any other `unified-chart-primitives` kit piece.
