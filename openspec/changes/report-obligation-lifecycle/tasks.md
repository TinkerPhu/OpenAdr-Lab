...
- [ ] 1.1 Write a failing unit test in `VEN/src/vtn.rs` asserting `http_error`'s returned
      `anyhow::Error` downcasts to a `VtnHttpError` (or equivalent) exposing the numeric HTTP
      status, for at least a 404 and a 409 case.
- [ ] 1.2 Implement `VtnHttpError` in `VEN/src/vtn.rs` and change `http_error` to construct it
      (keep the existing `Display` message format so all current assertions on error text keep
      passing) — per design.md D1.
- [ ] 1.3 Write a failing unit test in `VEN/src/services/test_support/mock_vtn.rs` /
...
- [ ] 1.4 Implement the `MockVtn` status-carrying error injection from 1.3.
- [ ] 1.5 Write a failing unit test in `VEN/src/services/obligation.rs`: a due obligation whose
...
- [ ] 1.10 Run `wsl cargo test -j 2 -p ven-app` under `wsl_lock` (see CLAUDE.md `wsl-lock`) —
      full green before moving on.

## 2. R-43 — Wire `append_report_sent` into real report-submission call sites (D3)

- [ ] 2.1 Write a failing integration test: submitting a report via `run_measurement_reports`
...
- [ ] 2.2 Implement the call: `tasks/sim_tick/publish.rs::run_measurement_reports` accepts
...
- [ ] 2.3 Write a failing integration test: a due obligation's report submission via
      `ObligationService::check_and_report` results in a `ReportSent` row.
- [ ] 2.4 Implement the call: `check_and_report` accepts `Option<Arc<dyn HistoryPort>>`
...
- [ ] 2.5 Write a failing test in `VEN/src/routes/reports.rs`: a successful `POST /reports` (and
      separately, a successful `PUT /reports/{id}`) results in a `ReportSent` row via the
      already-available `ctx.history`.
- [ ] 2.6 Implement the calls in `post_reports` and `put_report` (`VEN/src/routes/reports.rs`),
      guarded on `ctx.history.is_some()` like other optional-history call sites in this file's
      neighboring routes.
- [ ] 2.7 Confirm (unit or route test) that all three call sites are no-ops — not errors — when
      `history` is `None`, per the spec's "No HistoryPort configured" scenario.
- [ ] 2.8 Confirm end-to-end that `GET /history/reports` returns non-empty after a real
...
- [ ] 2.9 Run `wsl cargo test -j 2 -p ven-app` under `wsl_lock` — full green.

## 3. GB-21 — Fix capacity-reservation payload-type strings (D4)

- [ ] 3.1 Grep the repo for every occurrence of `IMPORT_CAPACITY_RESERVATION` and
...
- [ ] 3.2 Write/update failing unit tests in `VEN/src/controller/reporter.rs` asserting the
...
- [ ] 3.3 Update `VEN/src/controller/openadr_interface.rs` if it also matches/constructs these
      strings during obligation extraction — add/update its unit test(s) accordingly.
- [ ] 3.4 Update `tests/features/ven_reporting_out.feature` and
      ...
      type strings.
- [ ] 3.5 Update the doc/wiki occurrences found in 3.1
...
- [ ] 3.6 Run `wsl cargo test -j 2 -p ven-app` under `wsl_lock` — full green; run
