...
The SQLite `notifications` table SHALL gain `dedup_key` (nullable TEXT), `count`
(INTEGER, default 1), and `last_seen_at` (INTEGER unix seconds) columns via an
additive migration. Existing rows SHALL be backfilled with `count` 1 and
`last_seen_at` equal to `created_at`. Dedup state SHALL survive a VEN restart via
the existing ring-seeding from the store.
