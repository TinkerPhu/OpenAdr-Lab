...
- **THEN** `POST /auth/token` is called with `ven-1` / `ven-1`
...
- **WHEN** a BDD step creates a program with targets `["ven-1"]`
- **THEN** the program body sent to the VTN is `"targets": ["ven-1"]`

#### Scenario: Enrollment scenario verifies VEN visibility
- **WHEN** `GET /programs` is called with `ven-1` token
- **THEN** only programs where `ven-1` is in targets (or targets is empty) are returned
