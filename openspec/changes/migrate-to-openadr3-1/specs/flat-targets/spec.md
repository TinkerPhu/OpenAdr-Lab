...
- **WHEN** a program is created with targets `["ven-1", "ven-2"]`
- **THEN** `GET /programs/{id}` returns `"targets": ["ven-1", "ven-2"]`
...
- **GIVEN** an event exists with `targets: ["ven-1"]`
- **WHEN** `GET /events` is called with a token for `ven-1`
- **THEN** the event appears in the response

#### Scenario: VEN does not see event targeted to another VEN
- **GIVEN** an event exists with `targets: ["ven-2"]`
- **WHEN** `GET /events` is called with a token for `ven-1`
...
- **GIVEN** a program exists with `targets: ["ven-1"]`
- **WHEN** `GET /programs` is called with a token for `ven-1`
- **THEN** the program appears in the response

#### Scenario: VEN does not see program targeted to other VENs
- **GIVEN** a program exists with `targets: ["ven-2", "ven-3"]`
- **WHEN** `GET /programs` is called with a token for `ven-1`
